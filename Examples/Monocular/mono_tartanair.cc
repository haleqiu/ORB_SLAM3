...
int main(int argc, char **argv)
{
    if((argc < 4) || (argc >5))
    {
        cerr << endl << "Usage: ./mono_kitti path_to_vocabulary path_to_settings path_to_sequence path_to_save(optional)" << endl;
        return 1;
    }
...
void LoadImages(const string &strPathToSequence, vector<string> &vstrImageFilenames, vector<double> &vTimestamps)
{
    ...
    string strPrefixLeft = strPathToSequence + "/image_lcam_front/";
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            vstrImageFilenames.push_back(strPrefixLeft + path);
            i++;
